...
///////////
///// CALCULO DA PRODUÇÃO AJUSTADA PELAS EMISSÕES DAS EMPRESAS
//////////



EQUATION("Firm_Effective_Production_Adjusted_with_CO2_Tax")
/*
Ajusta a produção efetiva com o imposto baseado nas emissões de CO2.
*/
    v[0] = V("Firm_Effective_Production");                // Produção efetiva da empresa
    v[1] = V("Firm_CO2_Emissions");                       // Emissões de CO2 da empresa no período atual
    v[2] = V("CO2_Tax_Rate");                             // Parâmetro: Taxa de imposto sobre emissões de CO2 ----> 0,25
    v[3] = V("CO2_max");                                  // Valor máximo de emissões de CO2 permitidas ----> 450
    v[4] = 0;                                             // Inicializa o imposto

    // Calcula o imposto com base nas emissões
    if (v[1] > v[3])                                      // Se as emissões atuais ultrapassarem o valor máximo
    {
        v[4] = (v[1] - v[3]) * v[2];                      // Imposto baseado nas emissões excedentes
    }

    // Ajusta a produção efetiva com o custo do imposto
    v[5] = v[0] - v[4];                                   // Produção ajustada considerando o custo do imposto

    v[5] = max(0, v[5]);                                  // Garante que a produção não seja negativa
